...
#[tokio::main]
async fn main() {
    let ds = Datastore::new("file://database.db").await.unwrap();

    let ses = Session::for_kv().with_ns("testns").with_db("testdb");
    let ast = "
CREATE spielplan:1 SET name = 'myTT', datum = heute, heim = verein:wallau, gast = verein:kriftel;
CREATE spielplan:2 SET name = 'myTTT', datum = morgen, heim = verein:wallau_II, gast = verein:kriftel_II;
";
    let res = ds.execute(ast, &ses, None, false).await.unwrap();
    ...
    let res_qry = ds.execute(qry, &ses, None, false).await.unwrap();

    let x = res_qry.first().unwrap();
    let y = x.output().unwrap().last();
